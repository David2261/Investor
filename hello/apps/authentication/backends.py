...
class JWTAuthentication(authentication.BaseAuthentication):
	...
	def authenticate(self, request):
		...
		1) `None` - мы возвращаем `None`, если не хотим проходить аутентификацию. Обычно
		это означает, что мы знаем, что аутентификация завершится неудачей. Примером
		этого является случай, когда запрос не включает токен в
		заголовки.

		2) `(пользователь, токен)` - мы возвращаем комбинацию пользователь/токен, когда
		аутентификация прошла успешно.

		Если ни один из вариантов не выполняется, это означает, что произошла ошибка
		и мы ничего не возвращаем.
	...
	def _authenticate_credentials(self, request, token):
		try:
			...
			msg = 'Неверная аутентификация. Не удалось расшифровать токен.'
			raise exceptions.AuthenticationFailed(msg)

		try:
			user = User.objects.get(pk=payload['id'])
		except User.DoesNotExist:
			msg = 'Пользователь, соответствующий этому токену, найден не был.'
			raise exceptions.AuthenticationFailed(msg)
